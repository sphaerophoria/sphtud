...
pub fn BitSet(comptime expansion_alloc_info: rsl.ExpansionAllocInfo) type {
    return struct {
        ...
        const Storage = rsl.RuntimeSegmentedListConfigurableUnmanaged(u8, expansion_alloc_info);
        const Self = @This();

        pub fn init(prealloc: std.mem.Allocator, prealloc_size: usize, max_size: usize) !Self {
        ...
        pub fn appendByte(self: *Self, expansion_alloc: std.mem.Allocator, b: u8) !void {
        ...
        pub fn shrink(self: *Self, expansion_alloc: std.mem.Allocator, bit_size: usize) void {
            self.storage.shrink(expansion_alloc, std.mem.alignForward(usize, bit_size, 8) / 8);
        }

        // Rounds up to next 8
        pub fn grow(self: *Self, expansion_alloc: std.mem.Allocator, bit_size: usize, fill: bool) !void {
...
test "bitset sanity" {
    ...
    try bitset.grow(linear_alloc.allocator(), 3, true);
    ...
    try bitset.grow(linear_alloc.allocator(), 16, false);

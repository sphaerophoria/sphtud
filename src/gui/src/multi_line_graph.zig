...
pub fn multiLineGraph(comptime Action: type, alloc: sphrender.RenderAlloc, shared: *const Shared, retrievers: anytype) !gui.Widget(Action) {
    ...
    const texts = try alloc.heap.arena().alloc(gui.gui_text.GuiText(*SingleGraphState), retrievers.len);
    for (texts, graph_states) |*text, *cache_item| {
        text.* = try gui.gui_text.guiText(alloc, shared.guitext_shared, cache_item);
    }
...
const SingleGraphState = struct {
    ...
    pub fn getText(self: *@This()) []const u8 {
        ...
        // FIXME: Allow override of x text (e.g. nanosecond -> second)
        return std.fmt.bufPrint(&self.label_buf, "{d} -> {d}", .{ p.x, p.y }) catch return &self.label_buf;
    }
};
...
pub fn MultiLineGraph(comptime Retriever: type, comptime Action: type) type {
    return struct {
        ...
        fn render(ctx: ?*anyopaque, widget_bounds: gui.PixelBBox, window_bounds: gui.PixelBBox) void {
            ...
            const txfm =
                sphmath.Transform.scale(self.x_scale * 2, self.y_scale * 2)
            ...
            for (self.retrievers, self.graph_states) |r, gs| {
                ...
                if (gs.hover_point) |hp| {
                    any_hover = true;
                    const hover_txfm = sphmath.Transform.scale(self.shared.style.hover_ratio_size * 1 / self.x_scale, self.shared.style.hover_ratio_size * 1 / self.y_scale / self.zoom * graph_aspect)
                        .then(.translate(hp.x, hp.y))
                        .then(txfm);

                    self.shared.solid_color_renderer.render(self.shared.solid_color_full_screen_source, .{
                        .color = .{ color.r, color.g, color.b },
            ...
            for (self.retrievers, self.graph_states, self.labels) |r, gs, l| {
                ...
                self.shared.squircle_renderer.render(color, self.shared.squircle_corner_radius_px, color_bbox, gui.util.widgetToClipTransform(color_bbox, window_bounds));
            }
        }
        ...
        fn update(ctx: ?*anyopaque, available_size: gui.PixelSize, _: f32) anyerror!void {
            ...
            for (self.retrievers, self.graph_states) |*retriever, *graph_states| {
                const data_generation = retriever.getGeneration();

                const bounds = retriever.getBounds();
                max_x = @max(bounds.x, max_x);
                max_y = @max(bounds.y, max_y);

                if (data_generation == graph_states.generation) {
                    continue;
                }
                ...
                var gl_buf = try sphutil.RuntimeBoundedArray(sphrender.xyt_program.Vertex).init(self.shared.scratch.allocator(), retriever.len());
                ...
                graph_states.render_buf.updateBuffer(gl_buf.items);
                graph_states.render_source.setLen(gl_buf.items.len);
            }

            self.x_scale = 1.0 / max_x;
            self.y_scale = 1.0 / max_y;
        ...
        fn graphBoundsFromWidgetBounds(self: Self, widget_bounds: gui.PixelBBox) gui.PixelBBox {
            ...
            graph_bounds.bottom -= self.labelHeight();

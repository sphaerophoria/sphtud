...
glyph_buffer: Renderer.PlaneRenderProgram.Buffer,
...
pub fn init(alloc: Allocator) !GlyphAtlas {
    const program = try Renderer.PlaneRenderProgram.init(
        alloc,
        Renderer.plane_vertex_shader,
        Renderer.plane_fragment_shader,
        Renderer.DefaultPlaneReservedIndex,
    );
    ...
    const texture = Renderer.makeTextureOfSize(tex_width, tex_height, .rf32);
...
fn addCharToAtlas(self: *GlyphAtlas, alloc: Allocator, temp_alloc: Allocator, char: u8, point_size: f32, ttf: ttf_mod.Ttf, distance_field_renderer: Renderer.DistanceFieldGenerator) !UVBBox {
    ...
    const mask = Renderer.makeTextureFromR(canvas.pixels, canvas.width);
    defer mask.deinit();
...
fn renderTextureIntoAtlas(self: *GlyphAtlas, bounds: PixelBBox, distance_field: Renderer.Texture) !void {
    const render_context = try Renderer.FramebufferRenderContext.init(self.texture, null);
    defer render_context.reset();

    const temp_viewport = Renderer.TemporaryViewport.init();
    ...
    self.program.render(self.glyph_buffer, &.{}, &.{
        .{
            .idx = Renderer.DefaultPlaneReservedIndex.input_image.asIndex(),
            .val = .{ .image = distance_field.inner },
        },
    }, lin.Transform.identity);
}
...
const LinePointIter = struct {
    ...
    fn toClip(self: LinePointIter, in: FCoord) lin.Vec2 {
        const in_f: lin.Vec2 = @floatFromInt(in - self.bottom_left_fcoord);
        return in_f / self.scale * vec2Scalar(2.0) - vec2Scalar(1.0);
    }

    fn vec2Scalar(v: f32) lin.Vec2 {
        return @splat(v);
    }
...
fn makeDistanceField(
    ...
    width: usize,
    height: usize,
    mask_texture: Renderer.Texture,
    bbox: ttf_mod.BBox,
    distance_field_renderer: Renderer.DistanceFieldGenerator,
) !Renderer.Texture {
    var simple_glyph = try ttf_mod.glyphForChar(alloc, ttf, c) orelse return Renderer.Texture.invalid;
